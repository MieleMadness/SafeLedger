...
async function run() {
  ...
  await check('master password length policy supports long passwords', async () => {
    assert.strictEqual(runtimeUtils.MAX_MASTER_PASSWORD_LENGTH, 128);
    assert(60 <= runtimeUtils.MAX_MASTER_PASSWORD_LENGTH);
  });
  ...
  try {
    await check('first-run vault directory and encrypted profile creation', async () => {
    ...
    await check('wrong password remains an ambiguous crypto failure for policy classification', async () => {
    ...
    await check('valid ciphertext with damaged JSON is ambiguous until master key verifier checks it', async () => {
      const file = path.join(vaultDir, 'vaultlist.json');
      const original = fs.readFileSync(file, 'utf8');
    ...
    await check('atomic encrypted vault save leaves readable final file and no temp file', async () => {
    ...
    await check('password rotation commits new files before removing old files', async () => {
      const list = await robustVault.readVaultList(path.join(vaultDir, 'vaultlist.json'), oldKey);
      ...
      const newData = await robustVault.readVault(path.join(vaultDir, result.vaultList.vaults[0].file), newKey);
