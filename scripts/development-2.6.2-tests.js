...
assert.strictEqual(pkg.version, '2.6.2');
...
for (const name of ['Chain Games','Facebook','Yahoo','Google','Microsoft','Apple','Amazon','PayPal','Instagram','LinkedIn','GitHub','Netflix','Spotify']) {
  ...
  assert(serviceCatalog.iconDataUrl(service).startsWith('data:image/svg+xml;base64,'), `${name} icon must be fully local.`);
}
...
console.log('PASS SafeLedger 2.6.2 Shit Coin Mode, Chain Games, multichain asset identity, and known-site icon catalog gates.');
