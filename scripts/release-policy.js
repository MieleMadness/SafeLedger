...
function assertTagMatchesPackage(tag, pkg) {
  ...
  if (parsed.version !== pkg.version) throw new Error(`Tag/package version mismatch: ${parsed.tag} vs package ${pkg.version}`);
  return parsed;
}
...
function assertArtifactTemplates(pkg) {
  ...
  const macName = pkg?.build?.mac?.artifactName;
  if (winName !== 'SafeLedger-${version}-Portable.${ext}') throw new Error(`Unexpected Windows artifact template: ${winName || '(missing)'}`);
  if (linuxName !== 'SafeLedger-${version}-x86_64.${ext}') throw new Error(`Unexpected Linux artifact template: ${linuxName || '(missing)'}`);
  if (macName !== 'SafeLedger-${version}-macOS-arm64.${ext}') throw new Error(`Unexpected macOS arm64 artifact template: ${macName || '(missing)'}`);
}

function assertMasterAncestry(root) {
  try {
    execFileSync('git', ['merge-base', '--is-ancestor', 'HEAD', 'origin/master'], { cwd: root, stdio: ['ignore', 'ignore', 'pipe'] });
  } catch (err) {
...
if (require.main === module) {
  ...
  if (process.env.GITHUB_REF_TYPE && process.env.GITHUB_REF_TYPE !== 'tag') throw new Error(`Official release policy may run only in tag context; got ${process.env.GITHUB_REF_TYPE}.`);
...
module.exports = { SEMVER_TAG, REQUIRED_FILES, parseTag, assertTagMatchesPackage, assertRequiredFiles, assertArtifactTemplates, validateReleasePolicy };
