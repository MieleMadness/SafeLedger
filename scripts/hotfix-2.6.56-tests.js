...
assert(parts[0] === 2 && parts[1] === 6 && parts[2] >= 56,
  'SafeLedger 2.6.56 Recovery Command Center foundations must remain active on 2.6.56 and later 2.6.x candidates.');
assert(pkg.scripts['test:regression'].includes('node scripts/hotfix-2.6.55-tests.js'));
...
assert(dashboard.includes("'What Happens If…'"));
...
assert(summary.includes('simulationFacts: commandCenter.buildSimulationFacts(profileEntries)'));
assert(commandCenter.includes('buildSimulationFacts'));
for (const id of ['device-lost', 'safeledger-device-lost', 'location-unavailable', 'family-access', 'exchange-lockout']) {
  assert(simulator.includes(`id: '${id}'`));
}

assert(drill.includes("className = 'recovery-drill-progress-track'"));
...
for (const relative of [
  ...
  'src/main/data-write-service.js', 'src/main/motion-ui.js', 'src/main/column-collapse-ui.js',
  'src/main/security-ui.js', 'scripts/recovery-command-center-tests.js',
  'scripts/duplicate-asset-authoritative-tests.js', 'scripts/hotfix-2.6.56-tests.js'
]) execFileSync(process.execPath, ['--check', path.join(root, relative)], { stdio: 'pipe' });

console.log(`PASS SafeLedger ${pkg.version} keeps the 2.6.56 readiness ring, recovery scenarios, Emergency Package, authoritative duplicate protection, and reduced-motion-aware UI foundations active.`);
