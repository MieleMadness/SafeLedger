...
assert(parts[0] === 2 && parts[1] === 6 && parts[2] >= 2,
  'SafeLedger 2.6.2 release gates must apply to 2.6.2 and later 2.6.x patch releases.');

const readme = read('README.md');
assert(readme.includes('Current stable release: SafeLedger 2.6.2'),
  'README must identify SafeLedger 2.6.2 as the current stable release.');
...
assert(readme.includes('Windows x64 Portable EXE') && readme.includes('Linux x64 AppImage') && readme.includes('macOS Apple Silicon (`arm64`) ZIP'),
  'README must document all implemented 2.6.2 package targets.');
assert(readme.includes('Shit Coin Mode') && readme.includes('Chain Games'),
  'README must document the 2.6.2 asset-display and Chain Games additions.');
...
console.log(`PASS SafeLedger ${pkg.version} release documentation covers the 2.6.0 history, 2.6.1 Apple Silicon foundation, and 2.6.2 feature release.`);
