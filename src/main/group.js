...
function displayWalletName(name) { return normalize(name) === 'base app (coinbase wallet)' ? 'Coinbase Wallet' : (name || ''); }
...
function formatEasternDate(value) {
  ...
  try {
    return new Intl.DateTimeFormat('en-US', { weekday: 'short', month: 'short', day: '2-digit', year: 'numeric', hour: 'numeric', minute: '2-digit', second: '2-digit', hour12: true, timeZone: 'America/New_York', timeZoneName: 'short' }).format(date);
...
function renderReadinessCard(area, params) {
  ...
  message.className = 'recovery-readiness-message'; message.textContent = readiness.message; card.appendChild(message);
  const meta = document.createElement('p');
  meta.className = 'recovery-readiness-meta';
  meta.textContent = params.group.lastVerified ? `Last verified: ${formatEasternDate(params.group.lastVerified)}` : 'Last verified: Never';
  card.appendChild(meta);
  const actions = document.createElement('div'); actions.className = 'recovery-readiness-actions';
  const verify = document.createElement('button'); verify.type = 'button'; verify.className = 'btn btn-default btn-sm';
  ...
  verify.addEventListener('click', () => {
    ...
    params.saving.state = true; verify.disabled = true; statusMgr.loadStatus();
    ipc.send('process-group', { type: 'group-modify', vaultData: params.vaultData });
  });
  actions.appendChild(verify); card.appendChild(actions); area.appendChild(card);
}
...
const renderGroups = (params) => {
  ...
  if (params.vaultData && params.vaultData.groups) {
    const displayGroups = params.vaultData.groups.map((group, index) => ({ group, index })).sort((a, b) => normalize(displayWalletName(a.group.name)).localeCompare(normalize(displayWalletName(b.group.name))));
    const query = groupSearch && groupSearch.value ? groupSearch.value.toLowerCase() : '';
    for (const entry of displayGroups) {
      const i = entry.index; const current = entry.group; const category = getWalletCategory(current); const visibleName = displayWalletName(current.name) || 'Unnamed Wallet';
      const searchable = [visibleName, category, current.tags, getUserWalletNotes(current)].map((v) => String(v || '').toLowerCase()).join(' ');
      if (query && !searchable.includes(query)) continue;
      const li = document.createElement('LI'); const href = document.createElement('A');
      href.addEventListener('click', (e) => {
        e.preventDefault();
        if (params.saving.state) return alert('Please wait for processing to complete');
        params.vaultData.groupSelected = i; params.vaultData.recordSelected = null;
      ...
      const icon = document.createElement('span'); icon.className = 'glyphicon glyphicon-piggy-bank wallet-list-icon'; href.appendChild(icon);
      const text = document.createElement('span'); text.className = 'wallet-list-text';
      const name = document.createElement('span'); name.className = 'wallet-list-name'; name.textContent = visibleName; text.appendChild(name);
      if (category) { const sub = document.createElement('span'); sub.className = 'wallet-list-category'; sub.textContent = category; text.appendChild(sub); }
      href.appendChild(text); li.appendChild(href); ul.appendChild(li);
    }
...
const createEditGroup = (params) => {
  ...
  const inputName = editFormUi.addTextInput(grid, { id: 'inputName', label: 'Name', value: params.group ? displayWalletName(params.group.name) : '', maxLength: 25 });
  const inputCategory = editFormUi.addSelect(grid, { id: 'inputCategory', label: 'Wallet category', value: getWalletCategory(params.group) || '', options: ['', 'Hardware Wallet', 'Software Wallet', 'Other Wallet'] });
  const inputTags = editFormUi.addTextInput(grid, { id: 'inputTags', label: 'Tags (comma separated)', value: params.group && params.group.tags, maxLength: 250 });
  const inputPassword = editFormUi.addTextInput(grid, { id: 'inputPassword', label: 'Password', value: params.group && params.group.password, sensitive: true, revealLabel: 'password' });
  const inputPin = editFormUi.addTextInput(grid, { id: 'inputPin', label: 'PIN code', value: params.group && params.group.pin, sensitive: true, revealLabel: 'PIN code' });
  const inputRecoveryLink = editFormUi.addTextInput(grid, { id: 'inputRecoveryLink', label: 'Recovery link', value: params.group && params.group.recoveryLink, sensitive: true, revealLabel: 'recovery link' });
  const inputSeedPhrase = editFormUi.addTextInput(grid, { id: 'inputSeedPhrase', label: 'Seed phrase', value: params.group && params.group.seedPhrase, sensitive: true, revealLabel: 'seed phrase' });
  const inputNotes = editFormUi.addTextarea(grid, { id: 'inputNotes', label: 'Notes', value: getUserWalletNotes(params.group), rows: 4, maxLength: 500, className: 'detail-notes-input', full: true });
  const saveGroup = (button) => {
    if (params.saving.state) return alert('Please wait for processing to complete');
    if (!inputName.value) { if (button) button.disabled = false; return; }
    if (button) button.disabled = true;
    const g = params.group || { created: Date() };
    g.name = inputName.value; g.category = inputCategory.value; g.tags = inputTags.value; g.password = inputPassword.value; g.pin = inputPin.value; g.recoveryLink = inputRecoveryLink.value; g.seedPhrase = inputSeedPhrase.value; g.notes = inputNotes.value;
    ...
    ipc.send('process-group', { type: params.group ? 'group-modify' : 'group-create', vaultData: params.vaultData });
  };
  form.addEventListener('submit', (e) => { e.preventDefault(); saveGroup(null); });
  detailActions.set([{ icon: 'fa-save', title: 'Save wallet', className: 'detail-action-save', onClick: (_event, button) => saveGroup(button) }]);
};

exports.showGroupDetail = (params) => renderGroupDetail(params);
const renderGroupDetail = (params) => {
  const area = document.getElementById('detailArea'); area.innerHTML = '';
  const header = document.createElement('h1'); header.className = 'wallet-detail-title'; header.textContent = displayWalletName(params.group.name) || 'Wallet'; area.appendChild(header);
  ...
  const notesWrap = document.createElement('div'); notesWrap.className = 'detail-notes-section';
  const notesLabel = document.createElement('b'); notesLabel.textContent = 'Notes:'; notesWrap.appendChild(notesLabel);
  const notesValue = document.createElement('div'); notesValue.className = 'outData detail-notes-value'; notesValue.textContent = getUserWalletNotes(params.group); notesWrap.appendChild(notesValue); area.appendChild(notesWrap);
  appendDetailLine(area, 'Created', params.group.created, formatEasternDate); appendDetailLine(area, 'Modified', params.group.modified, formatEasternDate);
  detailActions.set([
    { icon: 'fa-pencil', title: 'Edit wallet', onClick: () => createEditGroup(params) },
    { icon: 'fa-print', title: 'Print recovery sheet', className: 'detail-action-print', onClick: () => securityUi.printRecoverySheet(`${displayWalletName(params.group.name) || 'Wallet'} Recovery Sheet`, [
      { label: 'Wallet', value: displayWalletName(params.group.name) }, { label: 'Category', value: category }, { label: 'Tags', value: params.group.tags }, { label: 'Password', value: params.group.password }, { label: 'PIN', value: params.group.pin }, { label: 'Recovery link', value: params.group.recoveryLink }, { label: 'Seed phrase', value: params.group.seedPhrase }, { label: 'Last verified', value: params.group.lastVerified ? formatEasternDate(params.group.lastVerified) : 'Never' }, { label: 'Notes', value: getUserWalletNotes(params.group) }
    ], true) },
...
const confirmDelete = (params) => {
  ...
  const note = document.createElement('p'); note.textContent = 'Use the red trash icon below to permanently delete this wallet.'; area.appendChild(note);
  detailActions.set([
    { icon: 'fa-times', title: 'Cancel delete wallet', className: 'detail-action-cancel', onClick: () => renderGroupDetail(params) },
    { icon: 'fa-trash', title: 'Confirm delete wallet', className: 'detail-action-delete', onClick: () => {
      params.vaultData.groups.splice(params.vaultData.groupSelected, 1); params.vaultData.groupSelected = null; params.vaultData.recordSelected = null; params.saving.state = true; statusMgr.loadStatus();
      ipc.send('process-group', { type: 'group-delete', vaultData: params.vaultData }); area.innerHTML = ''; detailActions.clear();
    } }
  ]);
};
