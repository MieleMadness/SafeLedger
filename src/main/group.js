...
function getCatalogWallet(group) {
  return walletCatalog.catalog.find((item) => normalize(item.name) === normalize(group && group.name));
}
...
const renderGroups = (params) => {
  ...
  if (params.vaultData && params.vaultData.groups) {
    ...
    const displayGroups = groupsArray
      ...
      .sort((a, b) => normalize(a.group.name).localeCompare(normalize(b.group.name)));
    ...
    for (const entry of displayGroups) {
      ...
      const searchable = [current.name, category, current.tags, getUserWalletNotes(current)]
      ...
      name.textContent = current.name || 'Unnamed Wallet';
