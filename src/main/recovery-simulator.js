...
function simulate(id, facts = {}, backupHealth = null) {
  ...
  if (id === 'device-lost') {
    const method = ratio(facts.recoveryMethodCount, total);
    const location = ratio(facts.recoveryLocationCount, total);
    const instructions = ratio(facts.recoveryInstructionCount, total);
    const score = Math.round((method * .35) + (location * .30) + (instructions * .35));
    return result(id, score,
      total ? `${Math.min(facts.recoveryMethodCount || 0, facts.recoveryLocationCount || 0, facts.recoveryInstructionCount || 0)} of ${total} Vault Items have all three core recovery ingredients documented.` : 'No Vault Items are documented yet.',
      ...
      [method < 100 ? `${total - (facts.recoveryMethodCount || 0)} Vault Item(s) still need a recovery method.` : '', location < 100 ? `${total - (facts.recoveryLocationCount || 0)} Vault Item(s) still need a recovery location.` : '', instructions < 100 ? `${total - (facts.recoveryInstructionCount || 0)} Vault Item(s) still need written recovery instructions.` : ''].filter(Boolean));
  }
  ...
  if (id === 'location-unavailable') {
    const score = total ? ratio(facts.separateLocationCount, total) : 0;
    return result(id, score,
      total ? `${facts.separateLocationCount || 0} of ${total} Vault Items document recovery and backup locations that appear different.` : 'No Vault Items are documented yet.',
      [score === 100 ? 'Every Vault Item has distinct documented recovery and backup locations.' : ''].filter(Boolean),
      [score < 100 ? `${total - (facts.separateLocationCount || 0)} Vault Item(s) may still rely on one documented location or do not list both locations.` : ''].filter(Boolean));
  }

  if (id === 'family-access') {
    const beneficiary = ratio(facts.beneficiaryCount, total);
    const instructions = ratio(facts.recoveryInstructionCount, total);
    const location = ratio(facts.recoveryLocationCount, total);
    const score = Math.round((beneficiary * .4) + (instructions * .35) + (location * .25));
    return result(id, score,
      total ? `${facts.beneficiaryCount || 0} of ${total} Vault Items identify a beneficiary or recovery contact.` : 'No Vault Items are documented yet.',
      ...
      [beneficiary < 100 ? `${total - (facts.beneficiaryCount || 0)} Vault Item(s) do not identify who should recover them.` : '', instructions < 100 ? `${total - (facts.recoveryInstructionCount || 0)} Vault Item(s) still need written instructions.` : '', location < 100 ? `${total - (facts.recoveryLocationCount || 0)} Vault Item(s) still need a recovery location.` : ''].filter(Boolean));
  }

  if (id === 'exchange-lockout') {
    const exchanges = Number(facts.exchangeCount) || 0;
    const documented = Number(facts.exchangeRecoveryPlanCount) || 0;
